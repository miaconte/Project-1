if(localStorage.getItem("memEvents") === null)
    localStorage.setItem("memEvents", []);
...
$(document).on("click", ".eventDiv", function () {
  ...
  //we can insert ",bar" after "type=night_club" to search for bars, but that also gives family restaurant stuff, which i dont think we want
  $.ajax({
      method: "GET",
      url: queryURL
  }).then(function (data) {
...
//     var toBeSaved = localStorage.getItem("memEvents");
//     toBeSaved.push({
//         eventTitle: "", //event title taken from selected concert
//         artists: "",    //event artists taken from selected concert
//         venue: "",      //event venue taken from selected concert
//         date: "",       //event date taken from selected concert
//         index:0,           //id in the array; we are about to reassign it
//         partyName: ""    //name of the after party club taken from selected club
//     });
//     toBeSaved[toBeSaved.length -1].index = toBeSaved.length-1;
//     localStorage.setItem("memEvents", toBeSaved);
//     $(this).hide();
// })

// $("#delBtn").on("click", function(){
//     var toBeSaved = localStorage.getItem("memEvents");
//     delIndex = ; //id taken from the selected concert or party
...
//     var dispArray = localStorage.getItem("memEvents");
//     for(let i = 0; i < dispArray.length; i++){
//         var leftDiv = $("<div>");
//         var rightDiv = $("<div>");
//         leftDiv.html("<b>"+ dispArray[i].eventTitle + "<br>" + dispArray[i].artists + "<br>" + dispArray[i].venue + "</b><br>"++""++"");
